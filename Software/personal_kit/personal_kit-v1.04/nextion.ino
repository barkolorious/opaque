...
void nextion_read (void) {
  if (nextion.available()) {
    ...
    while (nextion.available()) nextion_input += char(nextion.read());
    Serial.println(nextion_input);

    if (nextion_input[nextion_input.length() - 1] != mode) {
      Serial.println("Nextion error");
      while (nextion.available()) nextion.read();
      return;
    }
